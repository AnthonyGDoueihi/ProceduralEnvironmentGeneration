...
public class TextureCreatorWindow : EditorWindow {
    ...
    string filename = "myProceduralWindow";
    float perlinXScale;
    float perlinYScale;
    int perlinOctaves;
    ...
    private void OnGUI()
    {
        ...
        perlinXOffset = EditorGUILayout.IntSlider("Offset X", perlinXOffset, 0, 10000);
        perlinYOffset = EditorGUILayout.IntSlider("Offset Y", perlinYOffset, 0, 10000);
            ...
            for (int y=0; y<h; y++)
            {
                for (int x=0; x<w; x++)
                {
                    if (seamlessToggle)
                    {
                        ...
                        float noise00 = Utils.fBM((x + perlinXOffset) * perlinXScale,
                                                  (y + perlinYOffset) * perlinYScale,
                                            perlinOctaves,
                                            perlinPersistance) * perlinHeightScale;
                        float noise01 = Utils.fBM((x + perlinXOffset) * perlinXScale,
                                                  (y + perlinYOffset + h) * perlinYScale,
                                            perlinOctaves,
                                            perlinPersistance) * perlinHeightScale;
                        float noise10 = Utils.fBM((x + perlinXOffset + w) * perlinXScale,
                                                  (y + perlinYOffset) * perlinYScale,
                                            perlinOctaves,
                                            perlinPersistance) * perlinHeightScale;
                        float noise11 = Utils.fBM((x + perlinXOffset + w) * perlinXScale,
                                                  (y + perlinYOffset + h) * perlinYScale,
                                            perlinOctaves,
                                            perlinPersistance) * perlinHeightScale;
                    ...
                    {
                        pValue = Utils.fBM((x + perlinXOffset) * perlinXScale, (y + perlinYOffset) * perlinYScale, perlinOctaves,
                                                perlinPersistance) * perlinHeightScale;
